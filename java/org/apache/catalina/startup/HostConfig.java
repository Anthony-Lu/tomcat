...
public class HostConfig implements LifecycleListener {
    ...
    protected void deployApps() {
        ...
        // 是用来获取xml文件中配置。这个方法主要讲各个配置文件目录中的xml文件的内容读
        // 取出来，使配置的内容最终设置到容器中（比如Context的配置），这样配置内容就生效了
        deployDescriptors(configBase, configBase.list());
        // Deploy WARs
        deployWARs(appBase, filteredAppPaths);
        // Deploy expanded folders
        // deployDirectories()部署扩展的文件夹中的内容，比如一些扩展的webapp文件，其中xml配置文件也
        // 需要加载、web应用也要启动，和deployWARS方法的内容基本一致，
    ...
    private boolean isDeletableResource(File resource) {
        return (resource.getAbsolutePath().startsWith(
                host.getAppBaseFile().getAbsolutePath() + File.separator))
    ...
    public void start() {
        ...
        if (host.getDeployOnStartup())
            // 在start()主要处理了注册和类本身状态的设置之后，执行部署方法
            deployApps();
